...
static sgx_status_t SGX_CDECL sgx_seal(void* pms)
{
	...
	if (_tmp_debug != NULL && _len_debug != 0) {
		if ( _len_debug % sizeof(*_tmp_debug) != 0)
		{
...
err:
	if (_in_sealedData) free(_in_sealedData);
	if (_in_debug) free(_in_debug);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_sizeOfSealData, 0, 0},
		{(void*)(uintptr_t)sgx_seal, 0, 0},
		{(void*)(uintptr_t)sgx_encryptText, 0, 0},
		{(void*)(uintptr_t)sgx_decryptText, 0, 0},
	}
};
